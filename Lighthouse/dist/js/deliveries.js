var items = [];
var units = [];
var SelItemName, previousDate, nextDate, newDate, intDate, intTime, tzoffset, dateReceived, newQty;
...
app.controller('deliveriesCtrl', function($scope, $http, $uibModal, $log, $filter){
	...
	$scope.addItem = function(itemCode, itemName, qty, curQty, unit, proType){
		if(itemName === undefined || itemName == ""){
			...
			if(items.length != 0){
				if (checkItem(SelItemName, unit) == true){
					$uibModal.open({
						templateUrl: 'warning.html',
						controller: 'warningCtrl',
						size: 'md',
	...
	function checkItem(item, unit){
		for (var i = 0; i <= items.length; i++) {
			if(items[i] !== undefined ){
				if(item === items[i].name && unit === items[i].unit){
					return true;
					break;
				}
			} 
	...
	$scope.save = function(){
		...
		if(items.length === 0){
		...
		} else if ($scope.receiptNo === "" || $scope.receiptNo === undefined) {
			$uibModal.open({
				templateUrl: 'warning.html',
				controller: 'warningCtrl',
				size: 'md',
				resolve: {
					ErrorMsg : function () {
						return "Please Enter Reciept Number";
					}
				}
			});
			...
			$uibModal.open({
				...
				resolve: {
					confMsg : function (){
						return "Are You Sure Want You To Save Deliveries?"
					},
