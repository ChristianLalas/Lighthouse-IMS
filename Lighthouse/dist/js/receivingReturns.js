...
var app = angular.module('rcvrtns', ['ui.bootstrap', 'ngAnimate', 'ngSanitize']);

app.controller('rcvrtnsCtrl', function($scope, $http, $uibModal, $log, $filter){
	...
	$http.get(site_url + "/Receiving_Returns/getItems")
	...
	$scope.save = function(){
		...
		if(items.length === 0){
			...
			$uibModal.open({
				...
				resolve: {
					confMsg : function (){
						return "Are You Sure You Want To Save Returns?"
					},
					passData : function() {
						return data = [
							currentUser,
...
app.controller('confirmationCtrl', function($scope, $http, $uibModal, $uibModalInstance, confMsg, passData) {
	...
	$scope.yes = function(){
		$http.post(site_url + "/Receiving_Returns/insertRRtData", {
				"accoutID" 		: passData[0],
				"itemData" 		: passData[1],
				"trackID"		: passData[2],
				"issuer"		: passData[3],
				...
				window.location.assign(site_url + "/Receiving_Returns");
			}).error(function(data,status,headers,config){
				$uibModal.open({
					templateUrl: 'warning.html',
					controller: 'warningCtrl',
