...
var app = angular.module('givrtns', ['ui.bootstrap', 'ngAnimate', 'ngSanitize']);

app.controller('givrtnsCtrl', function($scope, $http, $uibModal, $log, $filter){
	...
	$http.get(site_url + "/Giving_BOReturns/getItems")
	...
	$scope.save = function(){
		...
		if(items.length === 0){
			...
			resolve: {
				confMsg : function (){
					return "Are You Sure You Want To Save Bad Order?"
				},
				passData : function() {
					return data = [
							currentUser,
...
app.controller('confirmationCtrl', function($scope, $http, $uibModal, $uibModalInstance, confMsg, passData) {
	...
	$scope.yes = function(){
		$http.post(site_url + "/Giving_BOReturns/insertGivBORtnsData", {
			"accoutID" 		: passData[0],
			"itemData" 		: passData[1],
			"trackID"		: passData[2],
			"issuer"		: passData[3],
			...
			window.location.assign(site_url + "/Giving_BOReturns");
		}).error(function(data,status,headers,config){
			$uibModal.open({
				templateUrl: 'warning.html',
				controller: 'warningCtrl',
